...
package object facets {
  ...
  private def addTextFacetField(doc: Document, fieldName: String, fieldValue: String): Unit = {
    if ( fieldValue.nonEmpty) { // Issues with empty strings on facets
      doc.add(new FacetField(s"${fieldName}${FacetedSearchRDD.FacetTextFieldSuffix}",
        fieldValue))
    }
  }
  ...
  def typeToDocument[T: ClassTag](doc: Document, fName: String, s: T): Document = {
    s match {
      ...
      case x: Long =>
        ...
        doc.add(new NumericDocValuesField(s"${fName} ${FacetedSearchRDD.FacetNumericFieldSuffix}",
          x))
      case x: Int =>
      ...
      case x: Float =>
        ...
        doc.add(new FloatDocValuesField(s"${fName}${FacetedSearchRDD.FacetNumericFieldSuffix}",
          x))
      case x: Double =>
        doc.add(new DoubleField(fName, x, Stored))
        doc.add(new DoubleDocValuesField(s"${fName}${FacetedSearchRDD.FacetNumericFieldSuffix}",
          x))
    }
    doc
  }
